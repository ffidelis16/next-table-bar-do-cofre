...
test("renderiza o wireframe RSVP com conteúdo essencial", async () => {
  ...
  assert.match(html, /<title>Nuvemshop Next no Bar do Cofre<\/title>/i);
...
test("mantém imagens e formulário do wireframe na marcação", async () => {
  const response = await render();
  const html = await response.text();
